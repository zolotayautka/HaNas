...
struct LoginView: View {
    ...
    private func attemptLogin() {
        ...
        Task {
            ...
            } catch let err as HaNasError {
                await MainActor.run {
                    isLoading = false
                    switch err {
                    case .httpError(let code):
                        if code == 401 {
                            errorMessage = NSLocalizedString("login_invalid_credentials", comment: "")
                        } else {
                            errorMessage = NSLocalizedString("login_failed", comment: "")
                        }
                    default:
                        errorMessage = "\(NSLocalizedString("connection_error", comment: "")): \(err.localizedDescription)"
                    }
                    showError = true
                }
            } catch {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    case .httpError(let code):
                        if code == 409 {
                            errorMessage = NSLocalizedString("register_duplicate_id", comment: "")
                        } else {
                            errorMessage = NSLocalizedString("register_failed", comment: "")
                        }
                    default:
