(async function(){
  ...
  authForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      authSubmit.textContent = t('loading');
  ...
  function handleFileClick(id, name){
    if(isImageByName(name)) {
      imageModal(id, name);
    } else if(isMediaByName(name)) {
      const mediaType = isMediaByName(name);
      playerModal(id, name, mediaType);
    } else {
  ...
  function imageModal(id, name){
    const src = '/file/' + id + '?inline=1';
    openModal(`<h3>${safe(name)}</h3><div style='margin-bottom:8px'><img src='${src}' style='max-width:100%;max-height:70vh;display:block;margin:0 auto' /></div><div class='foot'><button id='closeImage'>${t('close')}</button> <a href='/file/${id}' style='margin-left:8px'>${t('download')}</a></div>`);
  ...
  function render(node){
    ...
    children.forEach(c => {
      ...
      const isImage = !isDir && isImageByName(c.name);
      ...
      if(isDir){
        ...
        actions.push(`<button data-id='${c.id}' data-name='${c.name}' class='btnAction playBtn'>${t('play')}</button>`);
      ...
      if(!isDir) {
        actions.push(`<button data-id='${c.id}' data-token='${c.share_token || ''}' class='btnAction shareBtn'>${t('share')}</button>`);
      }
      actions.push(`<button data-id='${c.id}' class='btnAction deleteBtn' style='color:#900'>${t('delete')}</button>`);
      const row = `<tr>
        <td>${iconHtml}${nameHtml}</td>
        <td>${sizeHtml}</td>
        <td>${dateHtml}</td>
    ...
    listBody.querySelectorAll('.file-click').forEach(el=> el.addEventListener('click', (e)=>{ e.preventDefault(); const id=el.getAttribute('data-id'); const name=el.getAttribute('data-name'); handleFileClick(id, name); }));
    ...
    listBody.querySelectorAll('.shareBtn').forEach(b=> b.addEventListener('click', ()=>{ shareModal(Number(b.getAttribute('data-id')), b.getAttribute('data-token')); }));
  ...
  async function shareModal(id, existingToken){
    if(existingToken && existingToken !== '') {
      const shareUrl = `${window.location.origin}/s/${existingToken}`;
      openModal(`<h3>${t('shareLink')}</h3>
        <div style='margin-top:12px;margin-bottom:12px'>
          <input type='text' readonly value='${shareUrl}' style='width:100%;padding:8px;border:1px solid #ccc;border-radius:4px' id='shareUrl' />
        </div>
        <div class='foot'>
          <button id='copyBtn'>${t('copyLink')}</button>
          <button id='removeBtn' style='color:#900'>${t('removeShare')}</button>
      ...
      document.getElementById('copyBtn').addEventListener('click', ()=>{
        ...
        status.innerText = t('linkCopied');
      });
      document.getElementById('removeBtn').addEventListener('click', async ()=>{
        try{
          await apiPOST('/share/delete', {node_id: id});
          status.innerText = t('shareRemoved');
          closeModal();
          loadNode(currentNode && currentNode.id ? currentNode.id : undefined);
      ...
      openModal(`<h3>${t('createShareLink')}</h3>
        <div style='margin-top:12px;margin-bottom:12px;white-space:pre-line'>${t('createShareMsg')}</div>
        <div class='foot'>
          <button id='cancelBtn'>${t('cancel')}</button>
          <button id='createBtn'>${t('create')}</button>
        </div>`);
