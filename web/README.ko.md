...
React 기반의 HaNas 웹 프론트엔드입니다. iOS 클라이언트와 유사한 스타일로 디자인되었습니다.
...
개발 서버는 `/api` 경로를 `http://localhost:8080`으로 프록시합니다. 서버가 다른 포트에서 실행 중이라면 `vite.config.js`에서 프록시 설정을 변경하세요.
...
이 웹 클라이언트는 HaNas 서버와 함께 사용됩니다. 서버를 먼저 실행한 후 웹 클라이언트를 실행하세요.
